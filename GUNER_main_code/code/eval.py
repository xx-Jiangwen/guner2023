...
from transformers import BertModel, RobertaModel, RobertaConfig, BertConfig, BertTokenizer,AutoTokenizer,AutoModel
import sys
...
def build_transformer_model(bert_model_path, model_type='bert'):
    ...
    if model_type == 'bert':
        config = AutoTokenizer.from_pretrained(bert_model_path, num_labels=ent_type_size)
        encoder = AutoModel.from_pretrained(bert_model_path, config=config,output_hidden_states=True)
    if model_type == 'nezha':
        # logger.info("nezha")
