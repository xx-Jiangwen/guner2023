...
LastEditTime: 2023-05-24 20:47:20
'''
import copy
import os
...
def swa(model, model_dir, swa_start=1,swa_end=None ):
    """
    ...
    with torch.no_grad():
        for _ckpt in model_path_list[swa_start:swa_end]:
            ...
            model.load_state_dict(torch.load(_ckpt, map_location=torch.device("cuda:0")))
