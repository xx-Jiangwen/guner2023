...
def load_data(path):
    ...
    for d in json.load(open(path, encoding='utf-8')):
        D.append([d['text']])    #取文本
        for start, end, label in d['entity_list']:    #取实体的列表
            if start <= end:
                D[-1].append((start, end, ent2id[label]))     #[[text,(start, end, ent2id[label]),(start, end, ent2id[label]),(start, end, ent2id[label])]]
    return D
...
class EntDataset3(Dataset):
    ...
    def lowercase_and_normalize(self, text):
        ...
        text = text.lower()
        text = unicodedata.normalize('NFD', text)
        text = ''.join([ch for ch in text if unicodedata.category(ch) != 'Mn'])  #把文本中的基本拉丁字母和其变体形式全部合并为同一个字符，便于后续的处理。
        return text
    ...
    def get_token_mapping(self, text, tokens):
        ...
        for i, ch in enumerate(text):
            ...
            ch = ''.join([
                c for c in ch
                if not (ord(c) == 0 or ord(c) == 0xfffd or self._is_control(c))
            ])   #用于清洗输入文本，将无法识别的字符、控制字符和格式字符删除，使得输入文本规范化
            normalized_text += ch
            char_mapping.extend([i] * len(ch))

        text, token_mapping, offset = normalized_text, [], 0   #中文的字符串和字节表示是不同的
        for token in tokens:
            if self._is_special(token):
                token_mapping.append([])
            elif token == '[unused1]' or token == '[UNK]':
    ...
    def encoder(self):
        features = []
        if self.type != 'test':
            for (ex_index, item) in tqdm(enumerate(self.data)):
                text = item[0]
                tokens = []
                for t in text.split():
                    tokens += self.tokenizer.tokenize(t)
    ...
    def collate(self, examples):
        if self.type != 'test':
            ...
            #mappings = []
            for item in examples:
                text, input_ids, attention_mask, token_type_ids, start_mapping, end_mapping, label_, mapping = item
                ...
                labels = np.zeros((len(ent2id), max_len, max_len), dtype=np.int)
                for start, end, label in label_:
                    if start in start_mapping and end in end_mapping:
                        start = start_mapping[start]
                        end = end_mapping[end]
                        # print(labels[label, start, end])
                        if end < max_len and start < max_len:
                            labels[label, start, end] = 1
                batch_labels.append(labels[:, :len(input_ids), :len(input_ids)])
                #mappings.append(mapping)
            batch_input_ids = torch.tensor(self.sequence_padding(batch_input_ids)).long()
...
if __name__ == '__main__':
    ...
    # ner_train = EntDataset(load_data(args.train_file), tokenizer=tokenizer)
    ...
    time1 = time.time()
    for data in tqdm(ner_loader_train):
        #time.sleep(0.01)
        continue
    time2 = time.time()
